...
/*        CPFact cpFact=new CPFact();
        ...
        edge.getReturnVars().forEach(var -> {
            cp.meetValue(returnOut.get(var),returnValue);
        });
        if (edge.getCallSite().getDef().isPresent()&&edge.getCallSite().getDef().get() instanceof Var)
        cpFact.update((Var) edge.getCallSite().getDef().get(),returnValue);
        return cpFact;*/
